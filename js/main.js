...
function imgHover(event) {
  ...
  if (flag) {
    pathSrc = pathSrc.split('/').reverse()
    pathSrc[1] = 'norm'
    pathSrc = pathSrc.reverse().join('/')
    target.src = pathSrc
    console.log(pathSrc)
  }
}
function imgOutHover(event) {
  let target = event.target
  let pathSrc = target.src
  for (let index = 0; index < slides1.length; index++) {
    pathSrc = pathSrc.split('/').reverse()
...
disover1.addEventListener('mouseover', imgHover)
disover2.addEventListener('mouseover', imgHover)
disover1.addEventListener('mouseout', imgOutHover)
disover2.addEventListener('mouseout', imgOutHover)
